...
export const Products: CollectionConfig = {
      ...
      fields: [
        {
          name: "faqBlocks",
          type: "relationship",
          relationTo: "product-faqs" as any,
          hasMany: true,
          admin: {
            allowCreate: true,
            description: "Attach one or more FAQ blocks to this product",
          },
        },
        {
          name: "featureBlocks",
          type: "relationship",
          relationTo: "product-features" as any,
          hasMany: true,
          admin: {
            allowCreate: true,
            description: "Attach one or more Feature blocks to this product",
          },
        },
      ],
    },
  ...
  hooks: {
    beforeValidate: [
      ({ data }) => {
        // Remove falsy top-level id to avoid unique constraint errors during update
        if (data && (data as any).id == null) {
          delete (data as any).id;
        }
        // Some clients may leak Mongo-style _id; strip it as well
        if (data && (data as any)._id != null) {
